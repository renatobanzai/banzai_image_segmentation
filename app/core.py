...
class Image:
    ...
    def __init__(self, image_location):
        ...
        self.image_matrix = cv2.imread(image_location)
...
if __name__ == '__main__':
    ...
    img = Image("../data/talhao_101035/tci20201015_interest_area.png")
    #img.equalization_filter(img.image_matrix)
    #img.edges_and_links(img.image_matrix, 5, 1.4)


    gray = cv2.cvtColor(img.image_matrix,cv2.COLOR_BGR2GRAY)
    gray = img.equalization_filter(gray)
    ...
    markers = cv2.watershed(img.image_matrix, markers)
    img.image_matrix[markers == -1] = [255, 0, 0]
    cv2.imshow("imagem", img.image_matrix)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    print("aaa")